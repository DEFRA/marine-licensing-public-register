...
```bash
cd marine-licensing-public-register
nvm use
```
...
```bash
docker build --no-cache --tag marine-licensing-public-register .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 marine-licensing-public-register
```
